...
async fn handle_password_auth(
    ...
    //{{{
    sock.write_all(&[0x01, username.len() as u8]).await?;
    sock.write_all(username.as_bytes()).await?;
    sock.write_all(&[password.len() as u8]).await?;
    sock.write_all(password.as_bytes()).await?;
    sock.flush().await?;
    let mut buf = [0u8; 2];
